...
function App() {
  ...
  const [tasks, setTasks] = useState<Todo[]>(() => {
    return getFromLocalStorage<Todo[]>('tasks', [
  ...
  const [theme, setTheme] = useState<'light' | 'dark'>(() => {
    return getFromLocalStorage<'light' | 'dark'>('theme', 'light');
  });
  ...
  const sortedTasks = useMemo<Todo[]>(() => {
    let filtered = [...tasks];
  ...
  return (
    <ThemeProvider theme={isDarkTheme ? darkTheme : lightTheme}>
      <GlobalStyles />
      <StyledApp>
        <Header toggleTheme={toggleTheme} themeType={isDarkTheme ? 'dark' : 'light'} />
        <section id="center">
